...
from src.backend.codelist.boiler_agg_Obelisk import boiler_attribute_list
...
class ObeliskConsumer:
    ...
    def __init__(self, token = '<replace>') -> None:
    ...
    def format_given(self, data):
        # clean data 
        cleaned = self.__clean_boiler_aggregator_data((data))
    ...
    def __map_data(self, data: DataGrouped) -> ObeliskRequest:
        ...
        for el_d in data.metrics_boiler_aggregator:
            element_value = boiler_attribute_list[el_d]
    ...
    def __clean_data(self, df):
        ...
        # boiler.t_r::number, boiler.t_set::number, boiler.t_r_set::number
        df = df.loc[~((df['metric'] == 'boiler.t_r::number') & (df['value'] < 0))]
        df = df.loc[~((df['metric'] == 'boiler.t_set::number') & (df['value'] < 0))]
        df = df.loc[~((df['metric'] == 'boiler.t_r_set::number') & (df['value'] < 0))]
    ...
    def __clean_boiler_aggregator_data(self, df):
        """ Clean the received obelisk data
