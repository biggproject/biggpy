...
class BoilerAggregatorController:
    """ Class description: 
        The boiler aggregator controller makes a connection to the API of boiler aggregator and returns the raw value
    """
    ...
    def sign_in(self) -> str | None:
        ...
        log_info(f"GET {request_url}", Origin.BOILER_AGGREGATOR_CONTROLLER)
        r = post(request_url, data={'email': self.__env.email, 'password': self.__env.password})

        if r.status_code == 200:
            log_debug("Successfully authenticated", Origin.BOILER_AGGREGATOR_CONTROLLER)
            self.__token = r.json()['token']
            return self.__token
        else:
            log_error(f"Authentication failed! {r.status_code} | {r.text}", 'Boiler Aggregator Controller')
            return None
    ...
    def get_devicedata(self, request: DevicedataRequest, metrics : list[str]):
        ...
        if self.__token:
            ...
            if r.status_code == 200: 
                log_debug(f"GET {request_url} SUCCESS", Origin.BOILER_AGGREGATOR_CONTROLLER)
                return r.json()
            elif r.status_code == 401:
                log_debug("__token expired, getting new __token", Origin.BOILER_AGGREGATOR_CONTROLLER)
                self.sign_in()
                self.get_devicedata(request, metrics)
            else:
                log_error(f"Error {r.status_code}: {r.text}", Origin.BOILER_AGGREGATOR_CONTROLLER)
                return None
        else: 
            self.sign_in()
    ...
    def get_last_data(self, device_id: str):
        ...
        if self.__token:
            ...
            if r.status_code == 200: 
                log_debug(f"GET {request_url} SUCCESS : {r.json()}", Origin.BOILER_AGGREGATOR_CONTROLLER)
                return r.json()
            elif r.status_code == 401:
                log_debug("__token expired, getting new __token", Origin.BOILER_AGGREGATOR_CONTROLLER)
                self.sign_in()
