...
from src.backend.service.boilerAggregatorService import BoilerAggregatorService
...
class HouseService:
    ...
    """
    controller = BoilerAggregatorController()

    def put_t_set_house(self, house: str, t_set_value : int) -> bool:
        """ Newly fetched data from the boiler aggregator api will be stored in the storage
    ...
    def get_recent_houses_measurements(self, house_ids: list[str]) -> dict[str, pd.DataFrame]:
        ...
        for house_id in house_ids:
            if 'test' in house_id:
                ...
                # Load from boiler_aggregator
                boiler_aggregator = BoilerAggregatorService()
                ...
                log_debug(f"Getting measurements from boiler aggregator for period '{start}' - '{end}'")

                
                res = boiler_aggregator.get_device_data(DevicedataRequest(config['house_id_mappings'][house_id], start, end), metrics)
                if res == None:
                    return None

                df = pd.DataFrame(res.boiler_aggregator)
    ...
    def get_latest(self, house_ids: list[str], parameters: list[str] = ['t_r', 't_r_set','']) -> tuple[float, dict[str, float], dict[str, float]]:
        ...
        for house_id in house_ids:
            if 'test' in house_id:
                ...
                res: DevicedataResponse = boiler_aggregator.get_last_device_data(config['house_id_mappings'][house_id])
                
                try:
                    b_m_summed += (float(res.boiler[0]['blr_mod_lvl']) if float(res.boiler[0]['blr_mod_lvl']) > 0  else 0.0)
                    t_r_dict[house_id]=float(res.boiler_aggregator[0]['t_r'])
                    t_r_set_dict[house_id]=float(res.boiler_aggregator[0]['t_r_set'])
                except Exception:
                    return None
