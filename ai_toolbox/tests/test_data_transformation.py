...
class DataTransformation(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        filename = join(dir_path, "fixtures", "data_for_profile.csv")
        cls.df_profile = pd.read_csv(
            filename,
            sep=',',
            parse_dates=True,
        ...
        filename = join(dir_path, "fixtures", "result_profile.csv")
        cls.result_profile = pd.read_csv(
            filename,
            sep=',',
            parse_dates=True,
            infer_datetime_format=True,
    ...
    def test_yearly_profile_detection_raises_if_freq_none(self, mock_detect_time_step):
        ...
        self.assertRaises(ValueError, data_transformation.yearly_profile_detection, self.df_profile)
    ...
    def test_yearly_profile_detection_ok_if_freq_higher_than_daily(self, mock_detect_time_step):
        ...
        for frequency in ['60S', '1T', '30min', '15H', '1D']:
            ...
            assert_frame_equal(
                data_transformation.yearly_profile_detection(self.df_profile),
                self.result_profile,
                check_exact=False,
                check_dtype=False)
    ...
    def test_yearly_profile_detection_raises_if_exclude_days_wrong_type(self):
        ...
        self.assertRaises(TypeError, data_transformation.yearly_profile_detection, self.df_profile, "wrong_type")
    ...
    def test_yearly_profile_detection_ok_if_exclude_days_list(self):
        ...
        df_result_exclude = self.result_profile.drop(pd.Timestamp("2021-03-17 00:00:00+00:00"), axis="index")
        assert_frame_equal(
            data_transformation.yearly_profile_detection(self.df_profile, ["2019-03-17", "2020-03-17", "2021-03-17"]),
            df_result_exclude,
            check_exact=False,
            check_dtype=False)
    ...
    def test_yearly_profile_detection_ok_if_exclude_days_series(self):
        ...
        df_result_exclude = self.result_profile.drop(pd.Timestamp("2021-03-17 00:00:00+00:00"), axis="index")
        assert_frame_equal(
            data_transformation.yearly_profile_detection(self.df_profile, series_exclude),
            df_result_exclude,
            check_exact=False,
            check_dtype=False)
